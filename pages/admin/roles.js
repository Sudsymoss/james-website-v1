    ...
    <div className={styles.container}>
      ...
      <section className={styles.projects}>
        ...
        <form onSubmit={updateRole} className={styles.pform} ref={form}>
          <h2>Role update</h2>
          <select name="user" className={styles.ptitle} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required>
            <option value="" selected disabled>--select user--</option>
            {users.map(item => (
              <><option value={item.email} >{item.name}</option></>
            ))}
          </select>
          <select name="role" className={styles.ptitle} onChange={(e) => setFormData({ ...formData, role: e.target.value })} required>
        ...
        <form onSubmit={delUser} className={styles.pform} ref={form}>
          ...
          <button type="submit" className={styles.psubmitdel}>⚠DELETE USER⚠</button>
        </form>
      </section>
      <Footer/>
    </div>
